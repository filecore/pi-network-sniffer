...
bin/                        Orchestrator scripts (run on the Pi).
  ...
  wifi-survey.sh            Passive `iw scan` -- no association, no transmit.
  wait-for-network.sh       Blocks until default route + 1.1.1.1 reach.
  decide-vantage.sh         Snapshots public IP / CIDR / gateway / MAC.
  drain-queue.sh            Uploads queued offline runs at next online boot.
...
2. Flash Pi OS Lite (32-bit, Bookworm). Example hostname `blackbox`, user
   `pentest`, SSH key, optional Wi-Fi SSIDs.
...
The Wi-Fi survey is purely passive (receive-only `iw scan` -- no
association, no transmit). Active wireless attacks (deauth / handshake
capture / WPS / evil twin) are deliberately out of scope; see
`docs/setup.md` for the hardware and ethics rationale.
